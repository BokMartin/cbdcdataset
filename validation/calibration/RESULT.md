...
python scripts/evaluate_extraction_v10_1.py `
  --package data/calibration `
  --model-a data/calibration/openai_extractions.jsonl `
  --model-b data/calibration/claude_extractions.jsonl `
  --model-a-run-status protocol_exact `
  --model-b-run-status protocol_exact `
  --reference data/calibration/reference.csv `
  --out-dir <empty-output-directory> `
  --threshold 0.80
```

The evaluation outputs are deterministic. The committed reference uses repository-normalized LF line endings, so the recorded reference hash reflects those released bytes; its parsed contents are identical to the audited CRLF export.
